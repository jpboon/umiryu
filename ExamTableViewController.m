...
- (void) prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([segue.identifier isEqualToString:@"examSegue"]) {
        // send switch states and grade to next viewcontroller
        Exam *newExam = exam;
        
        ExamViewController *evc = [segue destinationViewController];
        NSString *tmpstr = [self.bands objectAtIndex:[self.tableView indexPathForSelectedRow].row];
...
{
    [super viewDidLoad];
    self.bands = [[NSArray alloc] initWithObjects:@"8.jpg",@"7.jpg",@"6.jpg",@"5.jpg",@"4.jpg",@"3.jpg", nil];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *currentImage = [self.bands objectAtIndex:indexPath.row];
